...
import { MailLabel } from 'app/mailbox/mailbox.types';
import { labelColorDefs, labelColors } from 'app/mailbox/mailbox.constants';
    ...
    (this.labelsForm.get('labels') as UntypedFormArray).controls.forEach(
      (labelFormGroup) => {
        ...
        if (labelFormGroup.dirty) {
          ...
          this._mailboxService
            .updateLabel(labelFormGroup.value.id, labelFormGroup.value)
            .subscribe();
        }
      }
    );
