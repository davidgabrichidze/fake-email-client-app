...
import {
  ...
  MailFolder,
  MailLabel,
} from 'app/mailbox/mailbox.types';
...
export class MailboxMailsResolver implements Resolve<any> {
  ...
  resolve(
    ...
    if (route.paramMap.get('folder')) {
      sources.push(
        this._mailboxService.getMailsByFolder(
    ...
    if (route.paramMap.get('filter')) {
      sources.push(
        this._mailboxService.getMailsByFilter(
    ...
    if (route.paramMap.get('label')) {
      sources.push(
        this._mailboxService.getMailsByLabel(
